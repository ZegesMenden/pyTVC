...
class test_vector_math(unittest.TestCase):
    ...
    def test_add(self):
        self.assertEqual(
            Vector3(0.0, 0.0, 0.0) + Vector3(1.0, 1.0, 1.0), Vector3(1.0, 1.0, 1.0)
        )
    ...
    def test_mul(self):
        self.assertEqual(
            Vector3(1.0, 1.0, 1.0) * Vector3(2.0, 2.0, 2.0), Vector3(2.0, 2.0, 2.0)
        )
    ...
    def test_div(self):
        self.assertEqual(
            Vector3(1.0, 1.0, 1.0) / Vector3(2.0, 2.0, 2.0), Vector3(0.5, 0.5, 0.5)
        )
    ...
    def test_norm(self):
        self.assertEqual(
            Vector3(5.0, 5.0, 5.0).normalize(),
            Vector3(0.5773502691896257, 0.5773502691896257, 0.5773502691896257),
        )

    def test_len(self):
        self.assertEqual(Vector3(5.0, 5.0, 5.0).length(), 8.660254037844387)
...
class test_Quat_math(unittest.TestCase):
    ...
    def test_eq(self):
        self.assertEqual(Quat(1.0, 0.0, 0.0, 0.0), Quat(1.0, 0.0, 0.0, 0.0))

    def test_mul(self):
        self.assertEqual(
            Quat(1.0, 0.5, 0.5, 0.5) * Quat(1.0, 0.5, 0.5, 0.5),
            Quat(0.25, 1.0, 1.0, 1.0),
        )

    def test_conj(self):
        self.assertEqual(
            Quat(1.0, 0.5, 0.5, 0.5).conjugate(), Quat(1.0, -0.5, -0.5, -0.5)
        )
    ...
    def test_rotate(self):
        ...
        q: Quat = Quat().from_euler(Vector3(0, 90 * DEG_TO_RAD, 0))
        print(q)
...
class test_body_math(unittest.TestCase):
    ...
    def test_rotation(self):
        ...
        for i in range(999):
            body.apply_torque(Vector3(0.0, 0.0, 1))
            body.update(0.001)
            body.clear()
        ...
        for i in range(999):
            body.apply_torque(Vector3(0.0, 0.0, -1.0))
            body.update(0.001)
            body.clear()
    ...
    def test_aero_forces(self):
        ...
        body: physics_body = physics_body(
            ...
            drag_coefficient_forewards=1.0,
            drag_coefficient_sideways=10.0,
            ref_area=1.0,
            use_aero=True,
        )
        ...
        self.assertAlmostEqual(body.drag_force, Vector3(0.6125, 0.0, 0.0), 4)

        body.velocity = Vector3(-100.0, 0.0, 0.0)

        body.update(0.01)
        if isinstance(body.drag_force, Vector3):
            self.assertAlmostEqual(body.drag_force, Vector3(6125.0, 0.0, 0.0), 4)
